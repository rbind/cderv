...
# https://github.com/yihui/rmarkdown-cookbook/blob/master/utils.R
...
* `knitr::all_labels()` which allows to retrieve all chunk label in a document. You can find a usage example in the R Markdown Cookbook about how to [Put together all code in the appendix](https://bookdown.org/yihui/rmarkdown-cookbook/code-appendix.html#code-appendix)
...
Then we would write our markdown content, for example using `knitr::knit_expand` as templating mechanism

```{r write-appendix, eval = FALSE}
```

This could be included in a single Rmd document like this 

`r import_example("_with-appendix.Rmd")`

And the resulting document would look like this 
